...
the `Combobox` and `Button` components (check out the `Code` tab in the "Clear
All Filters" story for a more elaborate example).
...
<DataList.Filters>
  ...
  <Button label="Clear filters" type="tertiary" onClick={removeAllFilters} />
</DataList.Filters>;
```
...
"Clear All Filters" story for a more elaborate example).
