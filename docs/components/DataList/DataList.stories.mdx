...
behaviour by including the DataList.LayoutActions component inside the
...
DataList.Filters add filters to the list. This compound component accepts any
children and drops it above the list of items.
...
By default, the DataList have a basic empty state message when it is empty or
filtering. You can override this by using the DataList.EmptyState component.
...
In order to select multiple items and perform an action on them, you have to set
up 2 things.
