...
selecting one or more options from a list. `Combobox` can be used for a variety
of scenarios, such as selecting items from a predefined list, filtering and
...
This is the default. When a selection is made, the Combobox will close.

#### Multi Select

Add the `multiSelect` prop for this behavior. This enables multiple options to
be selected and/or un-selected, and will only close when a user takes an
explicit action to close the open Combobox. eg. clicking outside the Combobox,
...
If immediate changes are not desired, and instead it is preferable to wait until
all selections have been made, `onClose` may be used.
...
- The options list is navigated with the `up` and `down` arrow keys, and
  actioned with `space` or `return`, this can be done at anytime when the
  Combobox is open
