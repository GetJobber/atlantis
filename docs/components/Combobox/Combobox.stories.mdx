...
If you want the Combobox to close after an action is clicked, you can use the
`closeOnActionClick` prop. An example of this behaviour can be found in the
[Close on Action Click](../?path=/story/components-selections-combobox-web--close-on-action-click)
story.
