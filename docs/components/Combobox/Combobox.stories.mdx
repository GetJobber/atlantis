...
The Combobox component supports several callback props for enhanced interaction:

- **`onSelect`** - Called when options are selected/deselected (optional)
- **`onSelectAll`** - Called when "Select all" is clicked in multiselect mode
  (optional)
- **`onClear`** - Called when "Clear" is clicked in multiselect mode (optional)
- **`onClick`** - Called when individual options are clicked (optional,
  available on ComboboxOption)

These callbacks provide additional hooks for custom behavior beyond the standard
selection handling.
