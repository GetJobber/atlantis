...
default activator or, you may pass in a custom activator.

##### Default activator

The default activator is a
[Chip](../?path=/story/components-selections-chip-web--subtle) containing an
`add` Icon suffix and an optional Combobox prop `label` that by default, reads
"Select". The label can be customized by adding the `label` prop to Combobox, to
better reflect the type of data being selected. Examples of using a default
...
The custom activator should be used when the Combobox activator is a
...
and will be rendered in place of the default activator. Examples of using a
