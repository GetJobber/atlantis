...
Snapshot tests are a common practice, but one we avoid in Atlantis. The idea
behind snapshots is great ("these tests will magically catch visual
regressions") but they leave too much to human interaction, and overall
understanding if it actually broke something, to be effective in practice.

Chances are, if you're updating a component and a snapshot fails, you may not
have the same context as the person that wrote it. In contrast, a true assertion
test tells you exactly what it is testing.

There's a much better way to write snapshot tests other than jest blob of
generated code since it creates more problems and work for us. In our experience
maintaining this system, we've never avoided a bug that snapshots caught, since
the snapshot is autogenerated and it's so easy to miss changes that are actually
breaking.

If, at some point, a regression does make it through existing tests and manual
QA, then we write more tests for that behaviour. Between this approach,
Typescript helping us avoid common mistakes, and being disciplined with our
intial test cases, we aim to maintain rigorous tests that catch regressions in a
