...
2. Auto-generate a new component called `AmountCounter` by running
...
Basically, It's annotated as `Semantic Name (HTML): Props Name`. This makes it
easier to write the code because we have an idea on what the HTML elements and
types of props the component could take. We'll ignore the card for now as
components should be pluggable anywhere and a card would restrict the overall
re-usability of this component.
