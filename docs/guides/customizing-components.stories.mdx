...
Some components expose their foundational building blocks. This usually
includes subcomponents and hooks for providing styles, state, and
additional logic.

These components provide greater flexibility, giving you much more control
over their appearance and behaviour.

### Example: [Button](../components/Button)

The `Button` component offers a very simple API allowing you to specify a
label, icon, and icon position. However, because it doesn't accept children,
...
* `useButtonContext`: Returns internal context that the `Button` shares with its subcomponents. Currently, only the Button's `size` is shared. This allows `Button.Icon` to inherit the same size and apply that to the underlying `Icon` it wraps. Consumers might use Button's context within a custom child component, allowing that child to adapt to the `size` prop for example.


## Named `customRender____` props

Some of our components use render props following the `customRender____`
naming convention.
