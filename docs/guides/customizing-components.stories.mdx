...
subcomponents. Currently, only the Button's `size` is shared. This allows
`Button.Icon` to inherit the same size and apply that to the underlying `Icon`
it wraps. Consumers might use Button's context within a custom child component,
allowing that child to adapt to the `size` prop for example.
