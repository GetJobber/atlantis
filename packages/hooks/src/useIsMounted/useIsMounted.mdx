...
`useLayoutEffect` that sets the `isMounted.current` to true when the component
mounts. When the component unmounts, `useLayoutEffect` calls a cleanup function
that sets `isMounted.current` to false.
