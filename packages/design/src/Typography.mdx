...
space between subsequent lines of type. Our `Heading` and `Text` components
handle their line-heights internally, so you don't need to worry specific
implementation values as long as you're working with those components.
