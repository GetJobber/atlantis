...
export function AutocompleteWithDetails() {
  const options = [
    {
      value: 1,
      label: "Sulaco",
...
export function ExampleWithSectionHeadings() {
  const options = [
    {
      label: "Ships",
      options: [
   ...
   */
  function filterOptions(text, _options = []) {
...
export function ExampleKitchenSink() {
  const options = [
    {
      label: "Ships",
      options: [
