...
[InlineLabel](/docs/components-inlinelabel--inline-label) and offers only 5
possible status representations:
...
For example, in a [List](/docs/components-list--list) where the StatusLabel is
on the right, use the `end` alignment.
...
- [InlineLabel](/docs/components-inlinelabel--inline-label) should be replaced
  with StatusLabel where possible to provide a more intentional communication of
  status to users
...
- StatusLabel should replace
  [InlineLabel](/docs/components-inlinelabel--inline-label) where possible.
