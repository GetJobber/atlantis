...
<Canvas>
  ...
  >
    {args => <StatusLabel {...args} />}
  </Story>
</Canvas>

<ArgsTable of={StatusLabel} story="StatusLabel" />

## Usage guidelines

StatusLabel is a more opinionated version of
[InlineLabel](../?path=/docs/components-inlinelabel--inline-label) and offers
only 5 possible status representations:
...
For example, in a [List](../?path=/docs/components-list--list) where the
...
- [InlineLabel](../?path=/docs/components-list--list) should be replaced with
  StatusLabel where possible to provide a more intentional communication of
  status to users
...
- StatusLabel should replace [InlineLabel](../?path=/docs/components-list--list)
  where possible.
