...
The `DataDump` above shows the date object that is returned, which includes the
values of a formatted date as a string, and a raw `Date` object (e.g.,
attributes can be accessed via `date.raw.getMonth()`). The date object's state
is managed internally by the component that is wrapped.
...
<Playground>
  {() => {
    ...
    return (
      <Content>
        <div>
          <DatePicker inline onChange={val => setDate(val)} />
        </div>
        {date && <DataDump defaultOpen data={date} />}
      </Content>
    );
