      ...
      return (
        <Content>
          <DatePicker
            ...
            onChange={setDate}
            activator={
              <Button label="My fancy activator" variation="learning" />
            }
          />
          {date && <DataDump defaultOpen data={date} />}
        </Content>
...
- If you are looking to use the `DatePicker` in a `Form`, consider the
  [InputDate](/docs/components-inputdate--input-date) component.
- For a time input, use [InputTime](/docs/components-inputtime--input-time)
- To present dates that have already been selected in a consistent format, use
  [FormatDate](/docs/components-formatdate--format-date)
