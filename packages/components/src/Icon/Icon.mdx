...
Only use `small` when space is severely constrained, such as in a [small Button.](/components/button/#small)
...
When contributing a new icon, design to these guidelines for the `base` size. The `small` 
and `large` sizes will be generated programatically once the `base` size icon has been
created.

#### Size, weight, and spacing

Icons are only responsible for their own spacing. Spacing when used within other components, 
such as [List](/components/list), is the responsibility of those other components.
...
- Icons can be optically adjusted within this 20px space to attain good visual balance with
  the rest of the Atlantis icon set
...
Design your icons with `#000000`. If an icon is associated with a systemic color such as ["destructive"](/colors#destructive) 
specify the system color for the icon when it is added to Atlantis. In all other cases, use the default icon color.
