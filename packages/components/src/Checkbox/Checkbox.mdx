...
Checkboxes let a user select one or more items from a set of options. If you
intending to turn an option on or off consider a [Switch](switch) instead.
...
<Playground>
  {() => {
    ...
    return <Checkbox checked={checked} label="Save card for future use" onChange={setChecked} />
