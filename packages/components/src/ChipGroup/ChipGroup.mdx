...
```ts
import { ChipGroup } from "@jobber/components/ChipGroup";
```

<Playground>
  <ChipGroup
    chips={[
      { icon: "checkmark", label: "hello" },
      { icon: "remove", label: "hi !", disabled: true },
    ]}
    disabled={true}
  />
</Playground>
...
The `<ChipGroup>` will... _Add a brief description of the component_
