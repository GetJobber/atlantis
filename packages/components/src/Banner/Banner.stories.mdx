...
[Content](/docs/components-content--content) to ensure consistent vertical
spacing between Banner and adjacent elements.
...
Otherwise, use [toasts](/docs/components-toast--toast) as the default for
success messages.
...
You should use
[ConfirmationModal](/docs/components-confirmationmodal--confirmation-modal) when
...
or, if unavailable,
[input validation](/docs/components-inputvalidation--input-validation).
...
- To provide low priority, temporary feedback on the outcome of a user action,
  use [Toast](/docs/components-toast--toast) instead
- Use
  [ConfirmationModal](/docs/components-confirmationmodal--confirmation-modal)
