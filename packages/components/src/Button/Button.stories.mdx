...
## Delighter and emphasis

The button animates the `icon` and `label` prop whenever you change them instead
of replacing the whole button with another button. This is a great way to draw
attention to a button and delight the user whenever they interact with it.
