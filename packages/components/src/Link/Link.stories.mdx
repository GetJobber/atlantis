...
A `<Link>` marked external will always open in a new tab and indicate with the
appropriate icon following the text. Set this when linking away from Jobber or
to other applications or sites across Jobber where this behaviour is appropriate
(e.g. linking from Jobber Online to the Jobber Help Center).
...
Contents of the link should be text, and may include some formatting features
(e.g. italic text for emphasis may be appropriate, changing the inherited
underline probably isn't).
