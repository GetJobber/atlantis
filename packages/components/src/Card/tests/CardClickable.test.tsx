...
describe("when focused", () => {
  it("should trigger the click event only after enter press", () => {
    ...
    screen.getByTestId("clickable-card").focus();
    fireEvent.keyUp(screen.getByTestId("clickable-card"), { key: "Tab" });
    expect(clickHandler).toHaveBeenCalledTimes(0);

    fireEvent.keyUp(screen.getByTestId("clickable-card"), { key: "Enter" });
  ...
  it("should trigger the click event only after spacebar press", () => {
    ...
    screen.getByTestId("clickable-card").focus();
    fireEvent.keyUp(screen.getByTestId("clickable-card"), { key: "Shift" });
    expect(clickHandler).toHaveBeenCalledTimes(0);

    fireEvent.keyUp(screen.getByTestId("clickable-card"), { key: " " });
    expect(clickHandler).toHaveBeenCalledTimes(1);
  });
});
...
describe("when not focused", () => {
  it("should not trigger the click event on enter press", () => {
    ...
    screen.getByTestId("clickable-card").blur();
    fireEvent.keyUp(screen.getByTestId("clickable-card"), { key: "Enter" });
    expect(clickHandler).toHaveBeenCalledTimes(0);
  });
