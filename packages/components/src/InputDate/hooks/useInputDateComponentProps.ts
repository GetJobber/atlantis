...
export function useInputDateComponentProps(props: InputDateProps) {
  ...
  const onClickOutside = useCallback(
    ...
    [value, onHideDatePicker, formFieldRef.current],
  );
  ...
  useEffect(() => {
    ...
    const valueFromDate = isValid ? formatter.format(date) : "";

    if (valueFromDate !== value) {
      setValue(valueFromDate);
    }
  }, [date, value, hasFocus]);
  ...
  function updateFromValue(
    ...
    const dateFromValue = new Date(translateMonth(newValue));
    ...
    if (
      ...
      (props.minDate
        ...
        : true) &&
      (props.maxDate ? dateFromValue.getTime() < props.maxDate.getTime() : true)
    ) {
      props.onChange(dateFromValue);
      !dateOnly && setValue(formatter.format(dateFromValue));
    } else if (!dateOnly) {
