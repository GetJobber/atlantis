...
const StatefulTemplate: StoryFn<typeof InputFile> = args => {
...
const FileSizeValidatorTemplate: StoryFn<typeof InputFile> = args => {
  const [files, setFiles] = useState<FileUpload[]>([]);
...
const MaxFilesTemplate: StoryFn<typeof InputFile> = args => {
  const [files, setFiles] = useState<FileUpload[]>([]);
...
const VariationsAndSizesTemplate: StoryFn<typeof InputFile> = args => {
  return (
    <Content>
...
export const CustomAllowedTypes = {
  render: StatefulTemplate,
  args: {
    allowMultiple: true,
...
export const WithCustomHintText = {
  render: StatefulTemplate,
  args: {
    allowMultiple: true,
    getUploadParams: () => Promise.resolve({ url: "https://httpbin.org/post" }),
...
const CustomDropzoneContentTemplate: StoryFn<typeof InputFile> = () => {
  const [files, setFiles] = useState<FileUpload[]>([]);
...
const CustomButtonContentTemplate: StoryFn<typeof InputFile> = () => {
  const [files, setFiles] = useState<FileUpload[]>([]);
...
export const CustomContentInButtonVariation = {
