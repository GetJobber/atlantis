...
Toasts have 3 variations, `default`, `success`, and `error`. Each should be used
...
`onClose` allows a callback when the toast closes. This can be done by clicking
the close button, or letting the Toast naturally close.
  ...
  <Button
    ...
    onClick={() =>
      showToast({
        ...
        onClose: () => console.log("I ate toast for breakfast")
      })
    }
  />
</Playground>
