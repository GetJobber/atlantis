...
<ArgsTable of={showToast} story="Toast" />
...
length or CTA, use [Banner.](/docs/components-banner--banner)
...
Use [Banner](/docs/components-banner--banner) and, where necessary, targeted
error messaging (such as
[InputValidation](/docs/components-inputvalidation--input-validation)) so that
the user can appropriately assess and recover from the error.
