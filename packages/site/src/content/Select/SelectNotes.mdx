...
`enableCustomSelect`. This enhances grouped menus in Chromium 123+ when the
browser supports `appearance: base-select`.

#### Alignment limitation (custom UI)

When using `enableCustomSelect`, the closed control (what you see as the
...
- `Select.OptionGroup`: group options under a `label`. Use with
  `enableCustomSelect` to opt into the customizable grouped menu in Chromium.
