import { Meta } from "@storybook/addon-docs";
import { Banner } from "@jobber/components/Banner";

<Meta title="Components/Forms and Inputs/InputValidation" />

# Input Validation

<Banner type="warning" dismissible={false}>
  90% of validation for inputs will be handled out-of-the-box by the [input
  component&apos;s
  validation](/storybook/web/?path=/story/components-forms-and-inputs-inputtext--validation-message)
  . Check that you cannot use the built-in validation before using this
  component.
</Banner>

An InputValidation component is used to display validation messages for an
input.

## Design & usage guidelines

InputValidation allows you to show the validation messages in cases where an

[input's built-in validation](/storybook/web/?path=/story/components-forms-and-inputs-inputtext--validation-message)
is not available, such as in `inline` usage.

For more information about `validations` using any of the Input components, see
the

[InputText](/storybook/web/?path=/story/components-forms-and-inputs-inputtext--validation-message)
documentation.
