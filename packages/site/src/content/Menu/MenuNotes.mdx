...
- **Menu.ItemLabel**: Renders the action label within a Typography (span)
  component. You MUST supply valid "phrasing" content, such as a string or a
  single qualifying element. Utility components that render down to a string
  (e.g. translation helpers) are acceptable. Responds to the `destructive`
  Menu.Item prop.
