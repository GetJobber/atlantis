...
**Required**  
Controls when the layout switches between horizontal and vertical arrangements.
Can be either a breakpoint size (`sm`, `md`, `lg`, `xl`) or a custom pixel
value.
...
**Optional**  
Controls the gap between elements. Accepts either a design token or a custom
value.
...
**Optional**  
Controls the maximum number of items that can appear in a row before wrapping.
